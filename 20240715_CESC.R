...
my_boxplot=function(dat,group,group_cols,test_method='kruskal.test',fill= "Group",
  ...
  # dat=TGF.score[tcga.subtype$Samples,'score']
  # group=tcga.subtype$Cluste
  data=data.frame(Group=group,value=dat)
  data=crbind2DataFrame(data)
  data=melt(data)
...
##01.#############
#dir.create('06_TGF')
TGF.gc=read.gmt("06_TGF/HALLMARK_TGF_BETA_SIGNALING.v2023.2.Hs.gmt")
TGF.gc=TGF.gc$gene
length(TGF.gc)
length(intersect(TGF.gc,rownames(tcga_mrna)))
setdiff(TGF.gc,rownames(tcga_mrna))

##1.1####
TGF=TGF.gc
TGF.score=t(ssGSEAScore_by_genes(gene.exp = tcga_mrna,genes = TGF))
#TGF.score=data.frame(Samples=rownames(TGF.score),score=as.numeric(TGF.score[,1]))
colnames(TGF.score)='TGF.score'
writeMatrix(TGF.score,'06_TGF/TGF.score.txt',row = T)

###1.3####
TGF.score.cli=data.frame(TGF.score=TGF.score[tcga_cli$Samples,],tcga_cli[,c('OS','OS.time')])
TGF.score.cli$group=ifelse(TGF.score.cli$TGF.score>median(TGF.score.cli$TGF.score),'High','Low')

TGF.score.roc=ggplotTimeROC(TGF.score.cli$OS.time,
                              TGF.score.cli$OS,
                              TGF.score.cli$TGF.score,mks = c(1,3,5))
TGF.score.roc
TGF.score.km.OS=ggsurvplot(fit=survfit(Surv(OS.time/365, OS) ~ group,
                                         data = TGF.score.cli),
                             data=TGF.score.cli,
                             conf.int = T,pval = T,risk.table = T,
                             fun = "pct",size = 1,surv.median.line = 'hv',
                             title='TCGA-CESC',legend.title='TGF score',
...
TGF.score.km.OS=mg_merge_plot(TGF.score.km.OS$plot,TGF.score.km.OS$table,nrow=2,heights = c(3,1),align = 'v')

TGF.score.km.OS
ggsave("06_TGF/TGF.score.km.OS.pdf",TGF.score.km.OS,height = 6,width = 6)
...
pdf('07_WGCNA/1.pdf',width = 10,height = 10)
...
#17
write.csv(tpm_T2.module$Modules,file = "07_WGCNA/WGCNA_Modules.csv",row.names = T)
pdf('07_WGCNA/2.pdf',height = 5,width = 6)
...
write.table(MODULE.gene.num,file = "07_WGCNA/MODULE.gene.num.txt",sep = "\t",quote = F,row.names =F)
pdf('07_WGCNA/3.pdf',height = 6,width = 6)
...
# Plot the result
pdf('07_WGCNA/4.pdf',height = 6,width = 12,onefile = T)
...
TGF.score
##choose 
tcga_cli_use <-cbind.data.frame(tcga_cli[tcga_cli$Samples,],TGF.score=TGF.score[tcga_cli$Samples,])
head(tcga_cli_use)
tcga_cli_use=tcga_cli_use[,-1]
colnames(tcga_cli_use)
tcga_cli_use.part=data.frame(TGF.score=tcga_cli_use[tcga_cli$Samples,c(11)])
...
pdf('07_WGCNA/5.pdf',width = 4,height =6)
...
# 849
write.table(tcga.wgcna.gene,file = "07_WGCNA/tcga.wgcna.gene.csv",sep = "\t",quote = F,row.names = F)
pdf('07_WGCNA/S_model_brown.pdf',width = 6,height = 6)
...
savePDF('08_gsea/Fig8.pdf',fig3,height = 12,width = 18)
...
tcga_model_data <- cbind(tcga_cli[, c("OS.time", "OS")],
                         t(tcga_exp[intersect(tcga.wgcna.gene,rownames(tcga_exp)), tcga_cli$Samples]))
dim(tcga_model_data)
...
GSE44001_model_data <- data.frame(GSE44001_cli[, c("OS.time", "OS")],
                                  t(GSE44001_exp[intersect(tcga.wgcna.gene,rownames(GSE44001_exp)), GSE44001_cli$Samples]))
...
write.csv(myd_exp_resampling[[1]],file = "09_model/myd_exp_resampling.csv")
###############
SamplingTime <- read.table("09_model/select_SamplingTime.txt",header = T,sep = "\t")
SamplingTime<- SamplingTime$SamplingTime
length(SamplingTime)
num <- SamplingTime[29]#OK  26：29 auc 不ok
...
write.csv(data.frame(cohort=rep(c('Train','Test'),c(length(tra.samples),length(test.samples))),
          ...
          '09_model/TCGA_clinical.csv',quote = F,row.names = F)


tra.cox=cox_batch(dat = tcga_exp[tcga.wgcna.gene,tra.samples],
...
tra.cox.fit
write.csv(tra.cox.fit,file = "09_model/tra.cox.fit.csv")
...
names(sig.coef)
length(names(sig.coef))
pdf('09_model/LASSO.pdf',height = 5,width = 10,onefile = F)
par(mfrow=c(1,2))
plot(fit1)
plot(cv.fit)
dev.off()
...
module.coxforest=ggforest(cox, data = tra.data, 
                          ...
                          noDigits = 2)
module.coxforest
ggsave('09_model/gene_forest.pdf',module.coxforest,height = 4,width = 9)

lan <- coef(cox)
lan
paste0(round(lan, 3), '*', names(lan),collapse = '+')
"0.539*ITGA5+0.403*SHF+-0.311*SNRPN"
write.csv(data.frame(gene=names(lan),coef=as.numeric(lan)),'09_model/gene_coef.csv',row.names = F)
####
###########
...
ggsave('09_model/gene_ Coefficients.pdf',p1,height = 4,width = 9)

##4.1########
risktype.col=c('#FFB200',"#77E4C8")
...
write.table(tcga.risktype.cli,file = "09_model/tcga.risktype.cli.txt",row.names = T,col.names = T,quote = F,sep = "\t")
########4.2########
...
pdf("09_model/all_tcga_roc.pdf",height = 8,width = 8)
...
p4
ggsave('09_model/P4c-k.pdf',height = 15,width = 15)
...
savePDF(p5ab,file = "10_immu/p10ab.pdf",height = 4,width = 10)


# ssgsea####
saveRDS(tcga_exp,"10_immu/tcga_exp.RDS")
tcga.immu.ssgsea=immu_ssgsea(tcga_exp)
colnames(tcga.immu.ssgsea)
icg.dat.RS=cbind(tcga.risktype.cli$Riskscore
                 #,tcga_model_data[tcga.risktype.cli$Samples,names(lan)]
                 ,tcga.immu.ssgsea[tcga.risktype.cli$Samples,])
...
pdf('10_immu/p10c.pdf',height = 12,width =12,onefile = F)
pheatmap(icg_cor_res$r,
             color = circlize::colorRamp2(c(-1, 0, 1), c('#433D8B', 'white', '#FF4C4C')),
             main="Heatmap", # 
             display_numbers = icg_cor_res.p, # 
             cluster_cols = F, # 
             cluster_rows = F,
             show_rownames = T, #
             show_colnames = T,
...
save.image(file = 'CESC_20240730_2.RData')
