...
filtered_signaling_name <- signaling_name[grep("METABOLISM|METABOLIC|glycolysis|Gluconeogenesis|Fatty_acid", signaling_name$`colnames(aucs_go)`, ignore.case = TRUE), ]
filtered_signaling_name <- filtered_signaling_name[!grepl("negative", filtered_signaling_name, ignore.case = TRUE)]
write_csv(as.data.frame(filtered_signaling_name),file = '04_immu_metabolism/filtered_signaling_name_METABOLIC.csv')
filtered_signaling_name <- read.csv("04_immu_metabolism/filtered_signaling_name_METABOLIC_by_hand.csv")
...
for(i in infla_group){
  ...
  cor[i,"METABOLIC cor"] = result$estimate
  cor[i,"METABOLIC Pvalue"] = result$p.value
...
cor
saveRDS(cor,file = "04_immu_metabolism/cor_GO_METABOLIC.RDS")

p2 = ggplot(subset(cor[infla_group,],subset = cor$`METABOLIC Pvalue`<0.05&cor$`METABOLIC cor`>0) ,aes(reorder(Group, `METABOLIC cor`), 
                                                                                                            `METABOLIC cor`, 
                                                                                                            color = `METABOLIC Pvalue`, 
                                                                                                            size = abs(`METABOLIC cor`)))+
  geom_point() +
  theme_bw() +
  ...
  coord_flip()
p2
ggsave("04_immu_metabolism/cor_metabolism.pdf",p2,width = 10,height = 6)
